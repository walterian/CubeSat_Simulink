...
for count = 1:1
    ...
    samplingrate = 2.5*1e9; % 2.5 gsps
    vi_rockbase = sqrt(exprnd(TBrockbase, 1, duration*samplingrate));
    vi_icebase = sqrt(exprnd(TBicebase, 1, duration*samplingrate));

    fname_rock = ['data/rock_voltage' num2str(count) '.mat'];
    save(fname_rock, 'vi_rockbase');
    fname_ice = ['data/ice_voltage' num2str(count) '.mat'];
    save(fname_ice, 'vi_icebase');
